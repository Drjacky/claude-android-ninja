...
class AuthViewModelTest {
    ...
    fun setup() {
        hiltRule.inject()
        viewModel = AuthViewModel(loginUseCase, mock(), mock())
    }
    
    @Test
    fun `when email is changed, ui state is updated`() = runTest {
        // Arrange
        ...
        assertEquals(testEmail, (state as AuthUiState.LoginForm).email)
    }
    
    @Test
    fun `when login is clicked with valid credentials, performs login`() = runTest {
        // Arrange
        viewModel.onAction(AuthAction.EmailChanged("test@email.com"))
        viewModel.onAction(AuthAction.PasswordChanged("password123"))
        
        coEvery { loginUseCase(any(), any()) } returns Result.success(
            AuthToken("token123", User("1", "test@email.com", "Test User"))
        )
...
### Navigation Tests:
```kotlin
// app/src/test/kotlin/com/example/AppNavigationTest.kt
class AppNavigationTest {
    
    @Test
    fun `auth navigator implementation navigates correctly`() {
        // Arrange
