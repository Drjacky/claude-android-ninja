...
Route tasks through the Quick Reference table and Workflow Decision Tree.

**Required:**

- Open only the reference file for the active task; read its table of contents and the target section. Forbidden: load a multi-thousand-line reference in full when one section covers the task.
...
- **Greenfield:** Workflow Decision Tree → "Creating a new project?"
...
**Outside-repo stop rules (do not substitute repo edits):** Play upload, tracks, rollout, `versionCode` - [android-ci-cd.md](references/android-ci-cd.md); Play Integrity prerequisites - [android-security.md](references/android-security.md); production `adb install` / `pm clear` - [testing.md](references/testing.md#agent-automation-adb-and-uiautomator).
...
→ Pick contacts without `READ_CONTACTS`: [android-permissions.md → Contact picker (privacy-first)](references/android-permissions.md#contact-picker-privacy-first)  
→ Inline picker UI: [android-permissions.md → Embedded Photo Picker](references/android-permissions.md#embedded-photo-picker)  
→ Target SDK 37 location: [android-permissions.md → Android 17 location privacy](references/android-permissions.md#android-17-location-privacy)
...
→ Add Media3 via `assets/libs.versions.toml.template` (`media3` ref, `media3-playback` bundle); align pins with [dependencies.md → Media3](references/dependencies.md#media3)  
...
→ Use [android-media.md → Playback preloading (Media3)](references/android-media.md#playback-preloading-media3)  
...
→ Studio system traces: [android-performance.md → Android Performance Analyzer (APA)](references/android-performance.md#android-performance-analyzer-apa)  
...
→ For trace-backed debugging rules (what to require from the user, what not to infer without artifacts), use [android-performance.md → Perfetto (system traces)](references/android-performance.md#perfetto-system-traces)
...
→ Release-only shrinker crashes: [android-debugging.md → R8 keep-rules troubleshooting](references/android-debugging.md#r8-keep-rules-troubleshooting) and [gradle-setup.md → R8 Keep-Rules Audit](references/gradle-setup.md#r8-keep-rules-audit)  
→ Unexplained background kill on some devices: [android-debugging.md → Process kill under memory caps](references/android-debugging.md#process-kill-under-memory-caps) and [migration.md → Memory limiter](references/migration.md#memory-limiter-all-apps-on-affected-devices)  
...
→ Start at [android-debugging.md → R8 keep-rules troubleshooting](references/android-debugging.md#r8-keep-rules-troubleshooting)  
→ Use [gradle-setup.md → R8 Keep-Rules Audit](references/gradle-setup.md#r8-keep-rules-audit) for redundant-library removal, impact hierarchy, subsuming-rule detection, reflection narrowing, and AGP 9 default-optimization re-audit
...
→ System traces in Studio: [android-performance.md → Android Performance Analyzer (APA)](references/android-performance.md#android-performance-analyzer-apa)  
→ For ANR, jank, or main-thread claims without measurements, follow [android-performance.md → Perfetto (system traces)](references/android-performance.md#perfetto-system-traces) before concluding cause  
...
→ Play Console blocks upload for identity verification: [android-ci-cd.md → Play developer verification](references/android-ci-cd.md#play-developer-verification) (outside-repo human step)  
...
→ Required: catalog `compileSdk` / `targetSdk` 37; pin `agp`, Gradle wrapper, `kotlin`, and `ksp` only after `./gradlew help` succeeds per [gradle-setup.md](references/gradle-setup.md) and [dependencies.md](references/dependencies.md); cleartext, loopback, CT, and URI grant rules per [android-security.md → URI grants on outbound intents](references/android-security.md#uri-grants-on-outbound-intents); adaptive large-screen layouts, `adjustResize` on the launcher Activity, and IME-after-rotation per [compose-patterns.md](references/compose-patterns.md); background audio/video via [android-media.md](references/android-media.md); [Android 17 location privacy](references/android-permissions.md#android-17-location-privacy); memory-limiter repro via [migration.md → Memory limiter](references/migration.md#memory-limiter-all-apps-on-affected-devices); Robolectric rules per [testing.md → Robolectric and SDK 37 (Android 17)](references/testing.md#robolectric-and-sdk-37-android-17) **only** when JVM tests use `RobolectricTestRunner`  
→ Forbidden: production-wide cleartext without domain-scoped Network Security Config; cross-process loopback without the API 37 permission where the platform requires it; background `MediaPlayer` / `AudioTrack` / raw `ExoPlayer` without Media3 `MediaSessionService` + `mediaPlayback` FGS + `MediaSession`; Robolectric releases older than 4.13 on current JDKs; assuming implicit URI grants on outbound intents outside `ACTION_SEND`, `ACTION_SEND_MULTIPLE`, and `ACTION_IMAGE_CAPTURE` (see [android-security.md](references/android-security.md#uri-grants-on-outbound-intents))  
